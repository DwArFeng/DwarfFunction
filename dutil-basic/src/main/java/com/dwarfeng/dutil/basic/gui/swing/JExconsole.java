 ...
 * 控制台提供两个流：输入流和输出流，与 {@link JConsole}不同的是，输入流变为阻塞式的，当输入流没有数据时，不会立即返回
 * <code>-1</code>，
 ...
 * 						// close()方法调用console.in.close()方法，因为控制台的输入流不响应该方法。
 * }
 * 
...
public class JExconsole extends JPanel {
	 ...
	 * 生成一个默认的控制台。
	 * <p>
	 * 控制台的最大行数为 <code>3000</code> 行，清除系数为 <code>0.1</code>， 最大回滚数量为
	 * <code>10</code>。
	 */
	public JExconsole() {
		this(3000, 0.1, 10);
	 ...
	 * 
	 * @param maxLine
	 *            指定的最大行数，需要 <code> 0 &lt; maxLine</code>。
	 * @param cleanRatio
	 *            指定的清除系数，需要<code> 0.0 &lt; cleanRatio &lt;= 1.0</code>。
	 * @param maxRollback
	 *            最大的输入回滚数量。
	 ...
	 * 
	 * @param maxLine
	 *            控制台的最大显示行数，需要 <code> 0 &lt; maxLine</code>。
	 * @throws IllegalArgumentException
	 *             入口参数不符合要求。
	 */
	 ...
	 * 
	 * @param cleanRatio
	 *            控制台的删除比率，需要<code> 0.0 &lt; cleanRatio &lt;= 1.0</code>。
	 * @throws IllegalArgumentException
	 *             入口参数不符合要求。
	 */
