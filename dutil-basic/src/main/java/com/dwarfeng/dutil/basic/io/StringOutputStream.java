...
public class StringOutputStream extends OutputStream {
	...
	public StringOutputStream(Charset charset, boolean autoFlush, int initialCapacity, float loadFactor)
		...
		this.byteBuffer = ByteBuffer.allocate(initialCapacity < 0 ? 0 : initialCapacity);
	}
	...
	private void mayAutoFlush(boolean endOfInput) throws IOException {
		if (autoFlush) {
			flushData(endOfInput);
		}
	}

	private final void ensureRemaining(int requiredRemaining) {
