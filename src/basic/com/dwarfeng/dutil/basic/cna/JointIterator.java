 ...
 * <p> 该迭代器连接多个迭代器，这些被连接的迭代器对顺序敏感。该连接迭代器的迭代顺序是：首先对其中独立的迭代器中的第一个
 * 中的元素进行迭代，当第一个迭代器中没有更多的元素时，进而迭代第二个迭代器的元素，以此类推，直到所有迭代器中的元素
 * 都完成迭代。
...
public class JointIterator<T> implements Iterator<T> {
	...
	public static class Builder<T> implements Buildable<JointIterator<T>>{
		...
		/**
		 * 向构造器中添加一个数组。
		 * <p> 该数组将转换为迭代器添加进构造器中，其迭代顺序和数组的顺序一致。
		 * @param array 指定的数组。
		 * @return 构造器自身。
		 * @throws NullPointerException 入口参数为 <code>null</code>。
		 */
