 ...
 * @since 0.1.0-beta
 */
public abstract class AbstractKeyListModel<K, V extends ElementWithKey<K>> implements KeyListModel<K, V> {

	/**抽象列表模型的侦听器集合。*/
	private final Set<KeyListObverser<K, V>> obversers;
	 ...
	 */
	public AbstractKeyListModel(Set<KeyListObverser<K, V>> obversers) {
	...
	protected void fireAdded(int index, K key, V value){
		for(KeyListObverser<K, V> obverser : obversers){
			if(Objects.nonNull(obverser)) obverser.fireAdded(index, key, value);
		}
	}
	...
	protected void fireRemoved(int index, K key, V value){
		for(KeyListObverser<K, V> obverser : obversers){
			if(Objects.nonNull(obverser)) obverser.fireRemoved(index, key, value);
		}
	}
	...
	protected void fireChanged(int index, K oldKey, V oldValue, K newKey, V newValue){
		for(KeyListObverser<K, V> obverser : obversers){
			if(Objects.nonNull(obverser)) obverser.fireChanged(index, oldKey, oldValue, newKey, newValue);
		}
	}
