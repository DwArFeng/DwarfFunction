...
/**
 * 键值列表模型。
 * <p> 模型可以容纳一组有序的拥有主键的元素，并提供一系列的操作方法。
 * 该模型同样可以对元素根据主键进行操作。该模型是一个 {@link List}实现。
 * @author DwArFeng
 * @param <K> 泛型K，代表元素键的类型。
 * @param <V> 泛型V，代表元素的类型。
 * @since 0.1.0-beta
 */
public interface KeyListModel<K, V extends ElementWithKey<K>> extends List<V>,
ObverserSet<KeyListObverser<K, V>>{
	 ...
	 * <p> （如果 fromIndex 和 toIndex 相等，则返回的列表为空）。
	 ...
	 * <p> 可以对 indexOf 和 lastIndexOf 构造类似的语句，而且 Collections 类中的所有算法都可以应用于 subList。
	 * <p> 如果支持列表（即此列表）通过任何其他方式（而不是通过返回的列表）从结构上修改，
	 * 则此方法返回的列表语义将变为未定义（从结构上修改是指更改列表的大小，或者以其他方式打乱列表，
	 * 使正在进行的迭代产生错误的结果）。 
