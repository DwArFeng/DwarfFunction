 ...
 * <p> 保存器拥有两种行为的保存方式：
 * <br>1、保存器进行保存，当遇到异常的时候，保存过程中止，并抛出异常。
 * <br>2、保存器进行保存，当遇到异常的时候，在集合中记录下这个异常，并且继续保存，等到全部保存完毕后，返回异常集合。
 * <p> 保存器被设计成一次性使用的类，当调用任意一种保存方法之后，该类则失效，因此不能反复调用保存方法。
 * @author DwArFeng
 * @since 0.1.0-beta
 */
public interface Saver<T> {
	...
	/**
	 * 向指定的对象中连续保存数据。
	 * <p>  该方法向指定的对象中持续保存数据，当保存过程中遇到任何异常，该方法会将异常记录在异常集合中，
