 ...
 * <br> foth包涉及代数、几何、统计、线性代数四大方面的内容，每个方面的内容都来自相对应的数学学科。
 * 
 * <p> <b>同步问题：</b>
 * 
 ...
 * <br>不过，可变对象并不是在任何时候都可编辑，比如一个可变对象接收 {@link com.dwarfeng.dfoth.algebra.FValue}对象，
 * 所以这个对象是可变的对象，然而在传入参数时，却传入几个常量，这样，虽然这个对象是可变对象，却没有任何方法能够修改
 * 这个对象。因此，可变对象继承 {@link com.dwarfeng.dfoth.DFoth}接口，并且通过调用
 * {@link com.dwarfeng.dfoth.DFoth#canModify()}方法来查询这个类究竟是否可变，由于该包中的实现类都是可变的，所以
 * 该包中的所有实现类都间接继承该接口。
 * 
 * <p><b>关于运算（尤其是代数）：</b>
 * <br> 数学包中的运算分为算式运算和值运算。算式运算是通过算式类将不同的对象构造成一个算式，比如代数中的 TODO 算式
 * 算式运算是在保留原有的对象的结构上进行的运算，因此，如果原有对象中含有可变对象，则算式运算也是可变的。
 * 另一种运算是值运算，比如线性代数包中的 {@link com.dwarfeng.dfoth.linalge.RowVector}，该类具有行向量的运算方法，这些方法是封装在类的内部的
 * 在调用这些方法时，会对对象内在的 {@link com.dwarfeng.dfoth.algebra.FValue} 对象进行取值，返回
 * 的新对象只保留运算之后的值，所以值运算时破坏性的，通过值运算返回的结果不在用于运算元素的特性，即使参与运算的
 * 对象时可变的，通过运算获得的结果对象也同样是不可修改的。
 * 
 * <p> 该包中的计算都是通过 {@link com.dwarfeng.dfoth.algebra.FValue} 进行的，因此，在计算过程中，会对其中的double
