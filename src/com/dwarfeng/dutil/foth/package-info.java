 ...
 * <br>不过，可变对象并不是在任何时候都可编辑，比如一个可变对象接收 {@link com.dwarfeng.dutil.foth.algebra.FValue}对象，
 ...
 * 在调用这些方法时，会对对象内在的 {@link com.dwarfeng.dutil.foth.algebra.FValue} 对象进行取值，返回
 * 的新对象只保留运算之后的值，所以值运算时破坏性的，通过值运算返回的结果不在用于运算元素的特性，即使参与运算的
 * 对象时可变的，通过运算获得的结果对象也同样是不可修改的。
 * 
 * <p> 该包中的计算都是通过 {@link com.dwarfeng.dutil.foth.algebra.FValue} 进行的，因此，在计算过程中，会对其中的double
