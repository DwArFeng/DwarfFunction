...
public interface ConfigModel extends ObverserSet<ConfigObverser>{
	...
	/**
	 * 获取配置模型中的当前值。
	 * <p> 当前值可能合法，也可能不合法，如果想要获得一个配置模型中指定配置键的合法值，请使用 {@link #getValidValue(ConfigKey)}。
	 * <p> 如果配置模型中不存在指定的配置键或者入口配置键为 <code>null</code>，则返回 <code>null</code>。
	 * @param configKey 指定的配置键。
	 ...
	 * <p> 该 <code>Set</code>视图受模型支持，所以模型中所作的更改均可以在此视图中反应出来。
	...
	/**
	 * 移除该模型中指定的配置键（可选操作）。
	 * <p> 如果指定的配置键为 <code>null</code>，或者不在该模型中，则不进行任何操作。
	 * @param configKey 指定的配置键。
	 * @return 该操作是否对模型产生了变更。
	 * @throws UnsupportedOperationException 如果配置模型不支持该操作。
	 */
	...
	/**
	 * 移除该模型中的配置键（可选操作）。
	 * <p> 该方法会试图移除指定配置键 Collection 中的所有配置键。
	 * <p> 如果指定的配置键不为 <code>null</code>，且不在该模型中，则进行移除操作。
	...
	/**
	 * 获取一个配置键的合法的值。
	 * <p> 如果指定的配置键在该模型中存在，则查看该配置键的当前值是否合法，
	 * 如果合法，则返回当前值；如果不合法，则返回默认值。
	 * <p> 如果指定的配置键在该模型中不存在，则返回 <code>null</code>
	 * @param configKey 指定的配置键。
	 * @return 该配置键的合法的值。
	 */
	...
	/**
	 * 获取一个配置键对应的固定属性。
	 * <p> 如果该配置键不存在或者为 <code>null</code>，则返回 <code>null</code>。
	 * @param configKey 指定的配置键。
	 ...
	 * <p> 当指定的配置键不为 <code>null</code>，且存在于该配置模型中时，即可更改其固定属性。
	 * <br> 如果配置属性更改，虽然配置键对应的当前值不会发生变更，但由于配置值检查器的更改，该值的有效性仍然有可能发生变化。
	 * <p> 虽然配置模型中拥有该方法，但不推荐使用，因为，配置固定属性本身就应该是不可变的。
	 * @param configKey 指定的配置键。
